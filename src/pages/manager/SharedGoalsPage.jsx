  ...
  return (
    <div className="animate-slide">
      ...
      {showForm && (<div className="modal-overlay" onClick={() => setShowForm(false)}><div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="form-group"><label className="form-label">Select Employees</label>{employees.map(emp => (<label key={emp.id} style={{ display: 'flex', gap: 8, padding: '6px 0', cursor: 'pointer' }}><input type="checkbox" checked={selectedEmps.includes(emp.id)} onChange={e => setSelectedEmps(e.target.checked ? [...selectedEmps, emp.id] : selectedEmps.filter(id => id !== emp.id))} />{emp.full_name}</label>))}</div>
