  ...
  return (
    <div className="animate-slide">
      ...
      {showForm && (<div className="modal-overlay" onClick={() => setShowForm(false)}><div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="form-group"><label className="form-label">Select Employees</label>{employees.map(emp => (<div key={emp.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', cursor: 'pointer' }} onClick={() => setSelectedEmps(prev => prev.includes(emp.id) ? prev.filter(id => id !== emp.id) : [...prev, emp.id])}><input type="checkbox" checked={selectedEmps.includes(emp.id)} readOnly style={{ pointerEvents: 'none' }} /><span>{emp.full_name}</span></div>))}</div>
